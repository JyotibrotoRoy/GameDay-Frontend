...
// Chaos Panel (Now accepts a className prop to fit into grid)
const ChaosPanel = ({ onTrigger, className }) => (
  <div className={`panel ${className}`}>
    ...
    <div style={{ padding: '1rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
      <button onClick={() => onTrigger('success')} className="btn-chaos" style={{ padding: '0.75rem', backgroundColor: 'rgba(20, 83, 45, 0.3)', border: '1px solid #14532d', borderRadius: '6px', color: '#4ade80', fontSize: '0.75rem', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}>GENERATE SUCCESS</button>
      <button onClick={() => onTrigger('latency')} className="btn-chaos" style={{ padding: '0.75rem', backgroundColor: 'rgba(120, 53, 15, 0.3)', border: '1px solid #78350f', borderRadius: '6px', color: '#fbbf24', fontSize: '0.75rem', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}>SIMULATE LATENCY</button>
      <button onClick={() => onTrigger('db-failure')} className="btn-chaos" style={{ padding: '0.75rem', backgroundColor: 'rgba(127, 29, 29, 0.3)', border: '1px solid #7f1d1d', borderRadius: '6px', color: '#f87171', fontSize: '0.75rem', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}>CRASH DATABASE</button>
      <button onClick={() => onTrigger('spike-memory')} className="btn-chaos" style={{ padding: '0.75rem', backgroundColor: 'rgba(88, 28, 135, 0.3)', border: '1px solid #581c87', borderRadius: '6px', color: '#c084fc', fontSize: '0.75rem', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}>SPIKE MEMORY</button>
    </div>
  </div>
);
...
export default function App() {
  ...
  const getApiUrl = () => {
    if (window.location.hostname !== 'localhost') {
      return 'https://bose-log-collector.onrender.com';
    }
  ...
  const handleChaos = async (type) => {
    try {
      await fetch(`${getApiUrl()}/api/chaos/${type}`, { method: 'POST' });
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const host = isProduction ? 'bose-log-collector.onrender.com' : 'localhost:8080';
    const protocol = isProduction ? 'wss' : 'ws';
    
    const client = new Client({
      brokerURL: `${protocol}://${host}/ws-logs/websocket`,
      onConnect: () => {
        setIsConnected(true);
        ...
        client.subscribe('/topic/metrics', (message) => {
          ...
          const rawMemory = data['UsedMemory'] || data['Used Memory'] || 0;
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="header">
        <div>
          <h1 className="title">
            <Activity color="#3b82f6" /> Game Day
          </h1>
          <p className="subtitle">Observability & Fault Injection Platform</p>
        </div>
      ...
      <div className="content-grid">
        ...
        <div className="panel panel-logs">
          <div className="panel-header">
            <div className="panel-title">
              <Terminal size={16} color="#94a3b8" /> Live Stream
            </div>
        ...
        <div className="panel panel-chart">
          <div className="panel-header">
            <div className="panel-title">Real-Time Memory Usage (MB)</div>
          </div>
